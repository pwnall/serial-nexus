...
//! producers on their channels. The pure `serial_nexus_core::data` contracts remain the
...
use serial_nexus_core::config::{GraphConfig, NodeConfig};
use serial_nexus_core::graph::{Arbitration, EndpointAddr, Facing, WriteMode};
use serial_nexus_core::lock::{EndpointLock, OriginId};
use serial_nexus_rpc::Notification;
...
pub(crate) async fn reacquire_held(lock: &SharedLock, id: OriginId) -> bool {
    ...
    // [`Acquire::Unregistered`](serial_nexus_core::lock::Acquire::Unregistered).
...
/// `serial_nexus_core::graph::MAX_NAME_LEN` = 256 bounds every channel identity, on every
...
impl Wiring {
    ...
    /// [`GraphConfig::effective_write_mode`](serial_nexus_core::config::GraphConfig::effective_write_mode)
...
mod tests {
    ...
    fn frame_payload_cap_reserves_the_envelope_header() {
        // 1 type byte + 2 channel-length bytes + the channel id (`serial_nexus_codec_api::encode`).
