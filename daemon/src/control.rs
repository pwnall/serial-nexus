        ...
        tokio::select! {
            ...
            line = lines.next_line() => {
                let line = match line {
                    ...
                    // guard, dequeuing a parked `lock --wait` promptly. (`serial-nexus-ctl`
...
mod tests {
    ...
    async fn a_pipelined_request_is_refused_without_killing_the_waiting_verb() {
        ...
        use serial_nexus_core::lock::{Arbitration, EndpointLock, OriginId};
        use serial_nexus_rpc::{Id, Incoming};
    ...
    // `itest/tests/p12_control_streams.rs` pins the same property end to end on
