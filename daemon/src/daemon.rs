...
use serial_nexus_core::graph::{EndpointAddr, WriteMode};
use serial_nexus_core::lock::{Acquire, Arbitration, OriginId, Steal};
use serial_nexus_rpc::{Notification, RpcError, error_codes};
...
/// once in [`serial_nexus_rpc::AppError`] (§16.8); these are const projections of that
...
impl Daemon {
    ...
    /// connection count and a `serial-nexus-ctl state` or a `tap`-only client would
    ...
    fn precheck_codecs(
        ...
        for nc in nodes {
            let serial_nexus_core::config::NodeConfig::Codec {
                codec,
                name,
                attributes,
                ..
    ...
    fn add_node(&self, params: Option<Value>) -> Result<Value, RpcError> {
        ...
        let mut node_cfg: serial_nexus_core::config::NodeConfig =
        ...
        if let serial_nexus_core::config::NodeConfig::Serial { device, .. } = &mut node_cfg {
            match self.resolver.resolve_input(device) {
                ...
                Err(serial_nexus_core::ResolveError::Malformed { input, reason }) => {
                    return Err(RpcError::invalid_params(format!(
                        "device {input:?}: {reason}"
                    )));
                }
    ...
    /// reads ([`Resolver::enumerate_ports`](serial_nexus_core::Resolver::enumerate_ports));
        ...
        self.state.with(|st| {
            ...
            let bound: Vec<(String, Option<std::path::PathBuf>, &str)> = st
                ...
                .filter_map(|n| match n {
                    serial_nexus_core::config::NodeConfig::Serial { name, device, .. } => Some((
                        name.clone(),
    ...
    /// [`serial_nexus_core::config::MAX_TIMER_MS`], which bounds a *retry* interval. `ms`
        ...
        let deadline = tokio::time::Instant::now()
            ...
            .unwrap_or_else(|| {
                tokio::time::Instant::now()
                    + Duration::from_millis(serial_nexus_core::config::MAX_TIMER_MS)
            });
    ...
    async fn wait_for_grant(
        ...
        let attempt = |g: &mut serial_nexus_core::lock::EndpointLock| match g.acquire(id) {
...
fn structural_errors(errors: &[serial_nexus_core::graph::ValidationError]) -> Option<RpcError> {
...
fn same_edge(
    x: &serial_nexus_core::config::EdgeConfig,
    y: &serial_nexus_core::config::EdgeConfig,
) -> bool {
    (x.a == y.a && x.b == y.b) || (x.a == y.b && x.b == y.a)
}

/// Parse `connect`/`disconnect` params into an [`EdgeConfig`](serial_nexus_core::config::EdgeConfig)
...
fn parse_edge_param(
    params: Option<&Value>,
) -> Result<serial_nexus_core::config::EdgeConfig, RpcError> {
...
mod tests {
    ...
    fn send_to_unserviceable_endpoint_fails_with_defined_error() {
        ...
        use serial_nexus_core::lock::EndpointLock;
