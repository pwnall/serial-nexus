...
impl IdentitySet {
    ...
    ///
    /// **This `Option` is a load-bearing convention, and it is the whole of the
    /// rate limit.** The accurate statement is not "no second predicate can fall
    /// out of step" — it is that there is *exactly one* predicate, it now lives in
    /// a different file from the `tracing::warn!` it gates, and the set's other
    /// caller (`leg::UnboundSet::insert`) discards this return and never logs, so
    /// nothing about the shape of the code says the `Some` arm is a first
    /// sighting. Widening it to answer `Some` for a repeat turns an operator-facing
    /// warning into a log flood at wire rate, on the single runtime thread, driven
    /// by whatever is on the wire — the exposure this module exists to bound.
    ///
    /// So it is pinned by a guard rather than by a comment:
    /// `codec::signal_tests::an_unconfigured_channel_warns_once_per_identity_however_loud_it_gets`
    /// captures the `tracing` events at the real call site and requires the logged
    /// names to equal [`Self::identities`] exactly, in order (plan §18 items 59(d)
    /// and 64).
    /// Until 2026-08-15 nothing asserted it: a repeat made to report itself as a
    /// first sighting left the daemon's own suite and fourteen itests green.
...
mod tests {
    ...
    /// **[`IdentitySet::insert`]'s `Option` is a first-sighting flag, and this is
    /// where that is written down** (plan §18 items 59(d) and 64).
    ///
    /// The cap, the dedup and the truncation are covered from both wrappers
    /// (`leg::UnboundSet`, `codec::UnconfiguredChannels`) and always have been. The
    /// *return value* was not covered anywhere: it is new with this module, it is
    /// what the codec's log-once WARN is gated on, and the leg — the other caller —
    /// discards it. So a widening here is invisible from the leg's side by
    /// construction, and was invisible from the codec's until the guard named in
    /// [`IdentitySet::insert`]'s doc landed beside it (plan §18 items 59(d) and 64).
    ///
    /// Two guards for one predicate is deliberate rather than duplication: this one
    /// pins the *convention*, deterministically and with no subscriber machinery;
    /// the codec's pins the *consequence*, that the WARN actually rides it.
    #[test]
    fn insert_answers_some_exactly_once_per_identity() {
        let mut set = IdentitySet::default();
        assert_eq!(set.insert("ch-a"), Some("ch-a"), "a first sighting");
        for _ in 0..100 {
            assert_eq!(
                set.insert("ch-a"),
                None,
                "a repeat is never a first sighting"
            );
        }
        assert_eq!(set.insert("ch-b"), Some("ch-b"), "a different identity is");

        // The `Some` carries the *stored* form, which is what a caller logs and what
        // `state` reports — never the caller's longer original.
        let long = "z".repeat(MAX_IDENTITY_LEN * 3);
        let stored = set.insert(&long).expect("a first sighting").to_owned();
        assert!(stored.ends_with(TRUNCATION_MARKER), "{stored}");
        assert_eq!(
            set.insert(&long),
            None,
            "the same identity, truncated the same"
        );

        // Past the cap nothing is recorded, so nothing may be reported as recorded.
        while set.identities().len() < MAX_IDENTITIES {
            let n = set.identities().len();
            assert!(set.insert(&format!("filler-{n}")).is_some());
        }
        for i in 0..50 {
            assert_eq!(
                set.insert(&format!("refused-{i}")),
                None,
                "an occurrence the cap refused is not a first sighting: the identity \
                 was never stored, so a caller logging on this arm would log a name \
                 that appears in no `state` snapshot — and would log one per frame, \
                 the refusals being unbounded in number"
            );
        }
