...
// [`READ_BUF`]-sized chunks) in `serial_nexus_core::config`.
...
async fn read_and_poll(
    ...
    // (`serial-nexus-doctor` P7 `latch_covers_termios_only_session: true` on 6.18 and
...
/// "absent" (serial-nexus-doctor P2: a never-opened master does not report POLLHUP).
