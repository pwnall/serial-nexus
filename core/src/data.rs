...
//! that state §5's rules precisely enough to property-test them. `serial-nexus-daemon`
...
//! tests (plus the `serial-nexus-itest` suite) are what cover *that* code. So a green
...
//!   Shipped as `serial-nexus-daemon`'s endpoint-keyed `runtime::Wiring::host_fanout`
