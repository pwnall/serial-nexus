...
test("adding a console through the editor makes bytes flow end to end", { tag: "@device" }, async ({
  ...
  try {
    ...
    //
    // Never let the cleanup throw *over* the body's failure. `finally` replaces an
    // in-flight exception with its own, so a `remove-node` that fails because the body
    // failed before creating the node reports the teardown and buries the cause — which
    // is exactly what happened once: a 15 s poll timeout surfaced as
    // "Command failed: … remove-node --cascade", naming the one step that was working
    // as designed. Report the cleanup problem; let the real error through.
    try {
      ctl("remove-node", name, "--cascade");
    } catch (err) {
      console.error(
        `[cleanup] remove-node ${name} --cascade failed: ${err.stderr || err.message}`,
      );
    }
  }
});
