...
async fn handle_conn<S: AsyncRead + AsyncWrite + Unpin + 'static>(
    ...
    // cannot omit it, while non-browser clients (`serial-nexus-web wsclient`, curl,
    ...
    // browser replays; the unscoped base name is for a headless client (`serial-nexus-web
...
async fn upgrade_ws<S: AsyncRead + AsyncWrite + Unpin + 'static>(
    ...
    let accept = serial_nexus_rpc::base64_encode(&hasher.finalize());
    let resp = format!(
