...
`systemd-run` can actually be run. Read there on 2026-08-22 (run 32551538826): image
`ubuntu-24.04` `20260816.277.1`, `PID 1:  systemd`, `uid:    1001   sudo: passwordless`,
systemd 255 (255.4-1ubuntu8.17). **One image and one systemd version**, which is the
scope of every cell below that cites it — not a claim about any other distro. Suite citations name the guard, which is the durable
...
**A guard that has not yet had a green run says so.** Until that run exists, a row
citing a root-arm guard keeps the class the claim is warranted by *today*, not the one
the guard will show — a citation to a run that has not happened is the same defect as a
measurement nobody took. Every row that carried that caveat is paid: the last four were
executed by CI run 32551538826's root arm on 2026-08-22 and each now cites it. The rule
stays written down because the next root-gated guard will land the same way.
...
| ↳ | the socket-group recipe in the unit's comment block (`groupadd`/`useradd`, `DynamicUser=no`, `User=`, `Group=`, and the `stat` output it predicts) | **measured** (both `stat` lines the block predicts — CI root arm, run 32551538826, 2026-08-22, `582c65f`, one `ubuntu-24.04` runner image); **measured** (the block's acceptance and its internal agreement) | Two halves, and they are not the same kind of thing. **Acceptance and agreement, measured on every push:** `p8_packaging.rs::the_socket_group_recipe_agrees_with_itself` parses the block out of the shipped unit and holds its halves to each other — the directives against the `groupadd`/`useradd` that create the identity they name, the predicted modes against `RuntimeDirectoryMode=`, the widening flag against `Group=` — and holds its two `stat` paths to the unit *around* it, the directory to `RuntimeDirectory=` and the socket to `ExecStart=`'s own `--socket`, because a verification step that agrees only with itself verifies nothing; eleven plants, each derived from the parsed recipe rather than typed as a literal, are each proven to redden it; and `::the_socket_group_recipe_verifies_clean_under_systemd_analyze` applies the recipe to the unit and verifies the result, which is the same **acceptance** the hardening row claims. **What the recipe produces was measured on 2026-08-22:** `::the_socket_group_recipe_hands_the_runtime_directory_to_the_operators_group` and `::the_socket_group_recipe_widens_the_control_socket_to_the_operators_group` run the block's own commands and check both predicted `stat` lines; they need root and a live systemd, so they self-skip everywhere but the CI root arm, and **that arm executed them for the first time on any machine** in CI run 32551538826 (`packaging` job, commit `582c65f`, image `ubuntu-24.04` `20260816.277.1`, systemd 255). The recipe's own `groupadd --system console-operators` + `useradd --system --no-create-home --shell /usr/sbin/nologin --gid console-operators --groups dialout serial-nexus` produced `dir_stat="serial-nexus console-operators 750"` and `sock_stat="serial-nexus console-operators 660"`, each equal to the prediction the block itself writes, and the daemon served `state` and shut down cleanly under it. **One runner image and one systemd version**, so the recipe is measured *there* and this row says nothing about another distro |
...
| The operators-group paragraph: `SupplementaryGroups=` cannot reach the runtime directory, so a static identity is the working recipe | man-page for the premise; **measured** (the recipe's acceptance and its internal agreement); **measured** (the runtime effect of both arrangements — CI root arm, run 32551538826, 2026-08-22, `582c65f`, one `ubuntu-24.04` runner image) | Same pair as the unit's socket-group row, and the same two guards. The paragraph's two halves are now measured *against each other* rather than separately: `p8_packaging.rs::the_socket_group_recipe_hands_the_runtime_directory_to_the_operators_group` runs the shipped unit with the operators' group added to `SupplementaryGroups=` **as a control** — asserting the group reaches the service's group list and still not the directory — beside the static identity that must produce the block's predicted `%U %G %a`. Either reading alone is consistent with the wrong story, which is why the guard takes both in one run. It needs root and self-skips everywhere but the CI root arm, which **executed it for the first time on any machine on 2026-08-22**, in CI run 32551538826 (`packaging` job, commit `582c65f`, image `ubuntu-24.04` `20260816.277.1`, systemd 255). The `SupplementaryGroups=` control read `dir_stat="snx-pkg-sockgrp-4200-shared snx-pkg-sockgrp-4200-shared 700"` with `groups="snx-pkg-sockgrp-4200-shared,dialout,console-operators"` — the operators' group in the service's group list and still not on the directory — while the static identity read `dir_stat="serial-nexus console-operators 750"`, the block's own prediction. Both readings in one run, which is what the pair was written for. **One runner image and one systemd version.** |
...
| The upgrade section's root `cp` procedure, step by step | **measured** (the copy and the adoption, off systemd); **measured** (the `/var/lib/private` path, and the three steps run under the packaged `[Service]` properties — CI root arm, run 32551538826, 2026-08-22, `582c65f`, one `ubuntu-24.04` runner image); **unverified** (the `systemctl` verbs against an installed unit) | linux-2026-08-21, by hand and recorded here with its box: a daemon started on a step-2 seed writes its snapshot at the `--state-file` path **mode 0600**; `cp` of a foreign snapshot onto that existing file leaves owner and mode unchanged — the invoking user and `600` both before and after, which is exactly what step 2's parenthetical promises; and the next start comes up reporting the copied graph's node and not the seed's. What that run did **not** do is any of the part that needs privilege — `systemctl start`/`stop`, and a copy into the root-only `/var/lib/private/` indirection this procedure exists for. `p8_packaging.rs::the_upgrade_procedures_root_copy_carries_the_snapshot_across` runs all three steps under the packaged unit's own `[Service]` properties and self-skips everywhere but the CI root arm, which **executed it for the first time on any machine on 2026-08-22**, in CI run 32551538826 (`packaging` job, commit `582c65f`, image `ubuntu-24.04` `20260816.277.1`, systemd 255). The seeding start left `state_file_stat="snx-pkg-upgrade-4200 600"` with `state_real="/var/lib/private/snx-pkg-upgrade-4200"`, so the root-only indirection is the path that was actually written; the root `cp` kept owner and mode (uid 63062, mode 600); and the second start read the same `600` and reported nodes `"carried,"` rather than the seed's. **The `systemctl` verbs stay unverified, and that is a stated bound rather than a missing run:** the probe hands the packaged `[Service]` directives to `systemd-run` as transient properties and never copies the unit into `/etc/systemd/system`, reloads, or starts it by name — a probe that installed a unit would overwrite a real deployment's, which the gate's own module doc lists among the things it does not catch. **One runner image and one systemd version.** The adoption behaviour itself is separately guarded by `p13_legacy_defaults.rs` (the row above) |
...
of privilege supplies one — it is carried separately as plan §18 item 78. The three
that wanted a **live systemd with root** were given one on 2026-08-22 and now read
**measured**. What survives of them is one clause of one row — the upgrade procedure's
`systemctl` verbs *against an installed unit*, which nothing here runs because
installing the unit would overwrite a real deployment's. That is a bound the gate
states in its own module doc, not a measurement anybody is owed.
...
**All four executed for the first time on 2026-08-22, in CI run 32551538826's root
arm** (`packaging` job, commit `582c65f`, image `ubuntu-24.04` `20260816.277.1`,
systemd 255): `test result: ok. 13 passed; 0 failed; 0 ignored`, with a `MEASURED` line
from each. They landed self-skipping, exactly as item 31 sequenced the first root
probe, and the rows above cite that run rather than promising it.

**A green job is not evidence that a gated test ran, and the two arms are what separate
them.** The same test binary runs twice here. Unprivileged it printed `SKIP` for exactly
these four names and for the older `DynamicUser=` probe; as root it printed `MEASURED`
for the same five. **Both arms ended `test result: ok. 13 passed; 0 failed; 0
ignored`** — so neither the job's conclusion nor the root arm's totals can tell a
measurement from a self-skip, and only the two arms read against each other can.
`SNX_PACKAGING_ROOT=required` is what makes the root arm's skip a red lane rather than
a quiet pass, which is the half of this that is a guard rather than a reading.
