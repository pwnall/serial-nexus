...
//! * `stat -c %s` → [`file_len`]; `jq` on `serial-nexus-ctl` text → structured RPC on
//!   `dump`/`state`; `sha256sum` + `tail -c` → [`sha256_hex`] over the in-memory tail;
//!   `timeout`/`sleep` → bounded [`wait_until`] and idle-drain polling.
//! * The bash read `replay_bytes` from a background `serial-nexus-ctl tap` process's
...
//!   `serial-nexus-sim pty --source` standing in for a UART), which is the Linux
