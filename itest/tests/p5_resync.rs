...
//!
//! `serial-nexus-sim mux --corrupt-every N` mangles one in every N emitted frames' type byte
...
//! `serial-nexus-sim mux` pty double — the pty-as-serial doctrine, which is Linux-only
