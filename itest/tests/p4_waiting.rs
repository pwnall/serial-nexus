...
//! control plane).
//!
//! The original stands a `serial-nexus-sim pty --sink` in for the device so the serial host
...
//! everywhere). Only the purge case needs a PTY client (a `serial-nexus-sim client` on `ptyb`),
...
use serial_nexus_itest::{Daemon, Rpc, Sim, TempRun, wait_until};

/// `serial-nexus-rpc` `AppError::Locked` (base `-32000` minus its ordinal `3`). A contended
...
/// `serial-nexus-ctl lock <origin> --wait`. It sends the request and keeps the socket
