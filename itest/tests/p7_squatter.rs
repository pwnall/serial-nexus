...
//!    adoption;
//! 4. the squatter (a byte-counting `serial-nexus-sim pty --sink`) received **0** bytes,
...
//!   `--dev-root <fixture>`; the "ours" device is a `serial-nexus-sim pty --echo` double at
...
//!   `serde_json` parse of the `serial-nexus-sim pty --sink` verdict (the sim subprocess is
...
/// Spawn the echoing "ours" serial device (a `serial-nexus-sim pty --echo`) at the fixture
...
/// Spawn the squatter as a byte-counting `serial-nexus-sim pty --sink` linked at the same
