...
//! §15.17 no-hardware).
//!
//! `serial-nexus-sim mux` feeds a reference-framed four-channel stream into a "device" PTY; a
...
//!
//! Platform: the multiplexed stream is driven through a `serial` node fed by a `serial-nexus-sim
...
mod linux_impl {
    ...
    use serial_nexus_itest::{Daemon, Sim, bin, wait_until};
    ...
    pub fn run() {
        ...
        // ---- Boot the daemon; a `serial-nexus-sim mux` pty double is the serial device ----
        let d = Daemon::start();
        let rpc = d.rpc();
        let run = d.run();
