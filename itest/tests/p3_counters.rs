...
//!
//! The "device" is a seeded `serial-nexus-sim` source/echo double, not hardware — the
...
use serial_nexus_itest::{Daemon, Rpc, Sim, TempRun, serial_echo, wait_until};
...
/// A single seeded-source serial device backed by `serial-nexus-sim pty --source` — the
...
/// `ENOTTY`); those checks then skip. Mirrors `serial_nexus_itest::serial_echo`. The returned
