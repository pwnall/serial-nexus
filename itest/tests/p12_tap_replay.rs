...
//!
//! Both need a software serial device (a `serial-nexus-sim` pty double), so both **skip** off
...
/// tap channel is empty. Duplicated from `daemon/src/tap.rs` deliberately: an
...
fn node_u64(rpc: &serial_nexus_itest::Rpc, node: &str, field: &str) -> u64 {
    rpc.node(node)
...
fn endpoint_u64(rpc: &serial_nexus_itest::Rpc, endpoint: &str, field: &str) -> u64 {
    rpc.state()["endpoints"]
        .as_array()
        .and_then(|a| {
            a.iter()
