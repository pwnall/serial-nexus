...
use serial_nexus_itest::{Daemon, Rpc, serial_echo, wait_until};

/// `AppError::Locked` — `APP_ERROR_BASE (-32000) - 3` (serial-nexus-rpc `AppError`).
...
fn park_a_waiter(
    ...
    origin: &str,
) -> mpsc::Receiver<Result<Value, serial_nexus_itest::RpcError>> {
