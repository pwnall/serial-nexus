...
//! `serial-nexus-sim wire --stall` peer streams sustained hostward on all channels but never
...
//!   `serial-nexus-sim wire` peer dials it (its `SimStream::connect` does not retry).
...
/// under `.channels.<id>.<field>` (see `daemon/src/nodes/leg.rs`).
