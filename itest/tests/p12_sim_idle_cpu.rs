...
//! slaves closed: `serial-nexus-sim`'s `pty --echo` and its `nullmodem` bridge. On a pty
...
//! therefore have been silent: no test measures a `serial-nexus-sim` process's CPU, and the
...
//!    `serial-nexus-doctor`'s P2 `hup_after_close`, read as a precondition. Without it the
...
//! *shipped* `serial-nexus-sim` binary's idle cost, so it catches a deleted or weakened
...
/// `serial-nexus-sim`'s `NO_SLAVE_PAUSE` doc — is ~223. Both doubles measured **1** tick
...
/// authority on it (AGENTS §7, `expectations/linux.jq`), and `serial-nexus-doctor` owns the
...
fn premise_holds(test: &str) -> bool {
    match pollhup_after_last_close() {
        ...
        Some(false) => {
            eprintln!(
                "SKIP {test}: this kernel no longer reports POLLHUP on a pty master \
                 after its last slave closes (serial-nexus-doctor P2 hup_after_close = false), \
        ...
        None => {
            eprintln!(
                "SKIP {test}: could not read serial-nexus-doctor P2, so the premise this \
