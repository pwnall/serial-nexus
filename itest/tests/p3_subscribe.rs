...
//!      (§7.2).
//!
//! No hardware: the "device" is a `serial-nexus-sim` PTY double standing in as a serial
...
use serial_nexus_itest::{Daemon, Sim, Subscription, serial_echo, wait_until};
...
/// The Rust replacement for `timeout N serial-nexus-ctl subscribe --count K`.
