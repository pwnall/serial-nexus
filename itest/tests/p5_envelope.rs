...
//!
//! `serial-nexus-sim envelope` drives an external codec child — here the stdlib-only Python
...
fn repo_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("serial-nexus-itest crate has a parent (the workspace root)")
        .to_path_buf()
}

/// Whether an external `python3` is on PATH (the child interpreter `serial-nexus-sim` will
...
fn envelope_passthrough_child_reemits_golden_battery() {
    ...
    // `serial-nexus-sim envelope --exec "python3 <passthrough.py>"`: the child reads envelope
    ...
    let out = Command::new(bin("serial-nexus-sim"))
        ...
        .arg(&exec)
        .output()
        .expect("run serial-nexus-sim envelope");
    let verdict: Value = serde_json::from_slice(&out.stdout).unwrap_or_else(|e| {
        panic!(
            "parse serial-nexus-sim envelope verdict: {e}; stdout={:?} stderr={:?}",
