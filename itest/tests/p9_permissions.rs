...
//! assertion here reads `std::fs::metadata` on a file a live `serial-nexus-daemon` created.
//!
...
fn a_leg_pointed_at_an_existing_file_refuses_and_leaves_it_intact() {
    ...
    let content = b"serial-nexus-itest: an operator file a leg must never unlink\n";
...
fn socket_group_chgrps_the_control_socket_and_widens_it_to_0660() {
    ...
    let rpc = serial_nexus_itest::Rpc::new(&socket);
    assert!(
