...
//! ["reference"]` (see `daemon/src/daemon.rs`) — so this port asserts the same
...
/// `AppError::Structural` — `APP_ERROR_BASE (-32000) - 2` (serial-nexus-rpc `AppError`).
...
fn info_reports_full_capability_surface() {
    ...
    // .instance | number — the per-boot nonce for tap offset-reset detection (plan §11.8).
    assert!(
