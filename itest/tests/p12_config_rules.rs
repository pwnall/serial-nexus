...
//!
//! Both fixes live in `GraphConfig::validate` (`core/src/config.rs`), which is
...
use serial_nexus_itest::{Daemon, Rpc, Subscription, serial_echo, wait_until};

/// `serial_nexus_rpc::AppError::Structural` — every `GraphConfig::validate` failure surfaces
/// with this code (§16.8).
...
///
/// The device is a `serial-nexus-sim pty --echo` double, so a frame the codec writes comes
...
// ---------------------------------------------------------------------------
// Local helpers (`serial-nexus-itest`'s shared lib is off-limits to this file, so the
// tap decoding lives here rather than in `src/lib.rs`).
// ---------------------------------------------------------------------------

/// Standard base64 decode — the inverse of the daemon's `serial_nexus_rpc::base64_encode`
