...
//! * The two `serial-nexus-sim pty --echo` "devices" become two [`serial_echo`] doubles.
...
//!   all flattened onto the node by the daemon) or a byte-exact `serial-nexus-sim client`
...
/// Returns the `serial-nexus-sim client` verdict (`pass`/checksum ground truth).
