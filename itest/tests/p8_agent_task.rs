...
//! ```text
//!   [ log "cap" ]──edge(never)──┐
//!                               ├─[ serial "usb0" exclusive ] ─▶ serial-nexus-sim pty --echo
...
use serial_nexus_itest::{Daemon, Rpc, serial_echo, sha256_hex, wait_until};
...
/// harness pulls in only `serial_nexus_itest` + std + `serde_json` (no `serial-nexus-rpc` dep).
