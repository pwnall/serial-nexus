...
//!   RPC over the control socket and `serial-nexus-sim` verdicts (the harness doctrine, §5).
//! * `serial-nexus-ctl … | jq` state polling becomes `Rpc::wait_status` / `Rpc::node`
...
//!   `--dev-root <fixture>`; the device is a `serial-nexus-sim pty --echo` double at the
...
/// Spawn the echoing serial device (a `serial-nexus-sim pty --echo`) at the fixture `/dev`
