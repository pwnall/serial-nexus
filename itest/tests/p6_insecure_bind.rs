...
/// The structural validation error code (`serial_nexus_rpc::AppError::Structural`, §16.8:
/// `APP_ERROR_BASE = -32000`, offset -2). Hardcoded as a literal because `serial-nexus-itest`
/// does not depend on `serial-nexus-rpc`; this mirrors the bash `grep -q -- "-32002"`.
