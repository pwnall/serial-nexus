...
//! `serial-nexus-sim`, because these tests need to own the exact moment the peer stops
...
/// A §9 `hello` frame in `serial-nexus-codec-api`'s wire layout:
/// `u32 body_len | u32 magic | u16 version | u32 capabilities | u16 count |
/// count × (u16 len | UTF-8 identity)`, all big-endian.
///
/// Hand-rolled: `serial-nexus-itest` does not depend on `serial-nexus-codec-api`, and a test that
...
/// Run `serial-nexus-sim` with `args` to completion and return its JSON verdict. The
...
fn sim_verdict(args: &[&str]) -> Value {
    let out = Command::new(bin("serial-nexus-sim"))
        .args(args)
        .output()
        .expect("run serial-nexus-sim");
    serde_json::from_slice(&out.stdout).unwrap_or_else(|e| {
        panic!(
            "parse serial-nexus-sim verdict: {e}; stdout={:?} stderr={:?}",
