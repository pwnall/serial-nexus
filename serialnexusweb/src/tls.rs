...
fn write_private(path: &Path, bytes: &[u8]) -> std::io::Result<()> {
    ...
    // and it closes the pre-placement hazard the `./serialnexusweb.key` default makes
    ...
    // file on disk; `nexus-daemon`'s state-file write does the same for the same
...
mod tests {
    ...
    /// back to `./serialnexusweb.{crt,key}` when `XDG_RUNTIME_DIR` is unset, so a local
