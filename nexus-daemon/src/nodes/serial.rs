...
/// `-serial pty`, `nexus-sim pty`) never reaches; for break, until somebody issues
...
/// `nexus-sim`) `ENOTTY`s, and the port used to be dropped still exclusive, which on
...
mod tests {
    ...
    // `nexus-itest/tests/p12_serial_exclusivity.rs`.
    // -----------------------------------------------------------------------
    ...
    /// `nexus-sim`) — fails after the claim is taken. The port must not be discarded
    ...
    fn a_hold_wakes_and_declines_the_moment_the_node_drops_its_port() {
        ...
        local.block_on(&rt, async {
            ...
            let held = tokio::time::timeout(
                ...
                handle.hold(nexus_core::config::MAX_TIMER_MS),
            )
            .await
