...
**Status:** Executed — phases 0–8, the simplification track (§9), the extension track (§10), and web-console items §11.1–§11.6 are complete, audited, and green; macOS is runtime-verified on real hardware and the validation suite is the Rust `nexus-itest` crate (§15.30, §15.31). §11.7–§11.9, §16.11, the console-map track (§12), the review-26 remediation track (§13 — all 93 findings dispositioned, ledger in `docs/historical/27-review-26-remediation-ledger.md`) and the graph-editing track (§14, from design §15.35) are executed. No open track.
