...
**Status:** Executed — phases 0–8, the simplification track (§9), the extension track (§10), and web-console items §11.1–§11.6 are complete, audited, and green; macOS is runtime-verified on real hardware and the validation suite is the Rust `nexus-itest` crate (§15.30, §15.31). §11.7–§11.9 and §16.11 are executed on the real rig (with the ring data-structure and doctor-P2 fixes the validation surfaced); open work: the console map track (§12, from design §7.8/§15.33).
