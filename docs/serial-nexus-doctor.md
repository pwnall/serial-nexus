...
| **P15** | Real-port flow-control honouring: does a named port **honour** a requested flow-control mode on read-back, or accept the request and silently drop it? Two modes on one open, and the `question` string names **both** since §15.59: the **hardware** mode (`CRTSCTS`) and the **software** mode (`IXON`/`IXOFF`, plan §18 item 14). (§7.1, §15.53) | **The interesting failure is the dishonest one, and this probe exists to separate it from the honest one.** A driver that *refuses* `CRTSCTS` is honest and reads `supported` here; a driver that returns success from `tcsetattr` and then reads the flag back clear is what `serial2`'s verify-by-read-back turns into `failed to apply some or all settings` on a `faulted` node, some time after `load` already returned success. `supported` in **two** named shapes, because a fleet can be honest without being uniform: every named port honoured it, so a `flow_control = "rts-cts"` edge configures here and the driver agrees it did; **or** some port refused the request outright, which is honest and which `load` deliberately does not reject — the verdict says which ports, and it must never claim an honour a refusing port did not give. *(This row read only the first sentence until 2026-08-12, which is the same false claim the shipped verdict made and the same one plan §18's alignment pass repaired in the probe.)* `degraded` in three distinct shapes, reported in this precedence: the probe could **not restore** the termios it found (nothing below it should be trusted — a reconfigured adapter is a worse outcome than an unanswered question), or `shipped_predicate_agrees` is `false` (this report and the daemon would answer differently about the same port — `serial_nexus_sys::honours_flow_control` is the predicate `load` consults, and a disagreement is worse than either verdict alone), or a port **accepted and dropped** the request. `skipped` with no `--port`, or where no named port could be opened. Never `unsupported`. **Opt-in behind `--port` like P3/P5/P11/P14, and deliberately not an observation on P11**: this question cannot be asked without *configuring* the port, and P11's stated contract is that it opens with the port's current settings unchanged and inspects rather than configures. So it runs after P11 — which therefore still sees the port as the operator left it — and before P14, and it restores the termios it found, checked rather than assumed (`baseline_restored`, and a failure to restore is the first `degraded` arm above). Measured on both arms: `supported` on Linux 7.0.0-29 with `cflag_before_hex` `0x10021cb2` → `cflag_after_hex` `0x90021cb2`, a delta of exactly `CRTSCTS`, on both ports (`docs/doctor/linux-7.0-2026-08-07-2b44c17-tier3.json`, binary `2b44c1700d17`, probe set `e79f5fcd86a2e5f0`, 2026-08-07), and `degraded` with `silently_dropped: true` on both ports of Apple's `IOSerialFamily` — `tcsetattr_ok: true`, `0x4b00` → `0x4b00` (`docs/doctor/macos-24.6.0-2026-08-05-acb5162-tier3.json`, binary `acb516289a4c`, probe set `82a8e2198e54626a`, 2026-08-05). A new id is a new question, so P15's arrival moved `probe_set` into a new era deliberately, exactly as P14's did (§15.44's era-close law). **The software half, and the two things it deliberately does not do** (plan §18 item 14). `xon-xoff` had no pre-check and no probe, while `serial2` verifies `c_iflag` by read-back exactly as it verifies `c_cflag` — so a driver accepting `IXON`/`IXOFF` and reading them back clear would fault a node with the same bare `failed to apply some or all settings` the `rts-cts` refusal exists to prevent, and **no artifact on any kernel said whether one does**. It does now: the reading is taken on P15's own open with P15's own restore and one more flag pair, which is what makes it an observation and not a second probe. **It landed in two steps, and both are worth reading.** At 2026-08-13 it shipped under a `question` string still naming `CRTSCTS` alone, so it moved `field_set` and **not** `probe_set` and it deliberately did **not** move the verdict — the only honest arrangement while the header asked less than the body answered, and a wording change is not worth an era boundary of its own while a real instrument change (P16) is already owed. At P16's landing the two were folded into **one** `probe_set` move, exactly as §15.59 planned: the `question` now names both kinds and the verdict degrades on a silently-dropped `IXON|IXOFF`, ranked **below** the `CRTSCTS` drop — an order now held for *stability*, so a two-defect headline does not move under a reader, and no longer because one finding is actionable and the other is not. *(It read "because that finding has a shipped consequence an operator acts on and this one has none" until 2026-08-14; §15.61 gave both the same consequence, and this sentence was one of the sites that repair missed — notes §3.101.)* **And then the daemon changed too** (§15.61, 2026-08-13): item 14's decline was *conditional* on a dropping driver being found — Apple's `IOSerialFamily` is one, accepting `IXON|IXOFF` and reading `c_iflag` back `0x0` → `0x0` on both ports, 6 of 6 (`docs/doctor/macos-24.6.0-2026-08-13-b346188-tier3.json`) — so §15.53's refusal was extended and the pre-check now consults this cell for `xon-xoff` exactly as for `rts-cts`. An accept-then-drop reading here therefore predicts a **structural refusal at `load`/`add-node`**, not the late fault it used to predict. *(This row read "**What did not change is the daemon**: §15.53's refusal still covers `rts-cts` only, no pre-check consults the software cell, and plan §18 item 14's decline stands" until 2026-08-14 — one of the sites §15.61's landing did not reach, notes §3.101, plan §18 item 72. The ranking below the `CRTSCTS` drop survives; its justification does not, since both findings now carry a shipped consequence.)* The other route by which the software pass moves the verdict is `baseline_restored`, which covers both flag words because a port left with `IXON` asserted is worse than any unanswered question. `serial2_readback_would_fault` is the cell that decides a node's fate, since it compares the whole `c_iflag` word rather than the two flags alone. Measured on Linux 7.0.0-29: `ftdi_sio` **honours** it — `iflag` `0x5` → `0x1405`, a delta of exactly `IXON|IXOFF`, on **both** ports of the FT232R crossover, with `serial2_readback_would_fault: false` and `baseline_restored: true` (`docs/doctor/linux-7.0-2026-08-13-8c00078-dirty-tier3.json`, binary `8c00078466c2-dirty`, probe set `e79f5fcd86a2e5f0`, 2026-08-13). **Darwin was owed and has landed** (2026-08-13): `IOSerialFamily` does the same to `IXON`/`IXOFF` as it does to `CRTSCTS` — `tcsetattr_ok: true` with `c_iflag` `0x0` → `0x0`, a delta of nothing, on both ports, 6 of 6 across three captures, `serial2_readback_would_fault: true` (`docs/doctor/macos-24.6.0-2026-08-13-b346188-tier3.json`). So both modes are now measured on both kernels, on the **same two adapters** and the same cable, and it is that pair — one driver dropping, one honouring — that makes the refusal a discrimination rather than a blanket rule. |
| **P16** | pts **slave-witness liveness**: does a held pts slave fd report `POLLHUP` once its master closes, and stay quiet while the master is open? Beside it, whether `stat` on the path the fd was opened through still resolves to the same device — the two candidate instruments, read side by side on one kernel in one run. (§15.59, notes §3.56/§3.60) | **This measures the harness rather than judging the kernel, and that is the unusual thing about it.** `itest`'s `SlaveWitness::prove_open` is the enforcement behind the seven guards notes §3.56 converted: it establishes that a held slave is still live by comparing `(st_dev, st_ino, st_rdev)` against a fresh `stat` of the path. That works **on Linux** because the kernel unlinks `/dev/pts/N` at the *master's* close — measured, `fstat(fd)` `Ok(020600)` beside `stat(path)` `ENOENT` on one closed pair (notes §3.60). On Darwin, whose `/dev/ttysNNN` are persistent devfs nodes, the same comparison was **expected to degrade** to a tautology plus the compile-time borrow — expected, never measured, which is the shape §7 forbids. This probe is the instrument notes §3.60 named and could not build, because `poll(2)` needs `unsafe` and `unsafe` lives only in `serial_nexus_sys` (§16.3). **Two arms, and each is the other's control** (§15.49): the quiet arm polls the held slave while the master is open, where `POLLHUP` must be *absent*, and the firing arm polls the same fd through the same mask after the master closes, where it must arrive. Either alone is uninformative — an fd that always reports a hangup answers "dead" correctly and "alive" wrongly, and a zero with no witness is not a measurement. The quiet arm runs twice, back-to-back and at the PTY node's own 5 ms `IDLE_POLL`, because "no hangup in back-to-back syscalls" and "no hangup at the mechanism's cadence" are different claims; both carry their wall clock in **microseconds**. `supported` when both arms answered; `degraded` when the control fired (reported **first**, because a hangup that was never absent makes the post-close reading unreadable), when the hangup never arrived, or on a probe error. Never `unsupported` — a kernel without a portable liveness signal is a fact to carry into the argument, not a contradicted premise — and never `skipped`: it is a pty probe and needs no `--port`. Read `poll_can_tell_a_live_pair_from_a_dead_one` beside `stat_comparison_can_tell`: those two are the answer, and `shipped_prove_open_would_refuse` is computed as the *shipped* disjunction so the report says what the harness's check would have done rather than what this probe thinks of it. Measured on Linux 7.0.0-29 (`docs/doctor/linux-7.0-2026-08-13-8c00078-dirty-p16-tier3{,-2,-3}.json`, binary `8c00078466c2-dirty`, probe set `4317ea5ac187f506`, 2026-08-13): quiet in 200 back-to-back and 64 paced passes, `POLLHUP` **1 µs** after the master closed (identical in all three runs) with `revents` `POLLERR|POLLHUP` and a following `read(2)` answering `eof`, `path_still_resolves: false` — so **both** instruments tell a live pair from a dead one here. **Darwin was owed, and it is the whole reason the probe exists — and it landed 2026-08-13 reading exactly the predicted row**: `stat_comparison_can_tell` `false` while `poll_can_tell…` reads `true`, so on that kernel `SlaveWitness::prove_open`'s stat checks degrade to a tautology and only the `POLLHUP` arm can answer (`docs/doctor/macos-24.6.0-2026-08-13-b346188-tier3.json`; item 66, notes §3.93). *(This sentence still said the capture was owed until 2026-08-14 — notes §3.101.)* A new id is a new question, so P16's arrival moved `probe_set` into a new era deliberately, exactly as P14's and P15's did (§15.44's era-close law) — and P15's `question` widening rode with it in the same commit, so the archive gained one boundary rather than two (§15.59). |
...
### All thirteen probes, 2026-08-05 — the `a131e1f4b46d6c83` era's 7.0 baseline, and the first lawful P13-era diff

**That era is CLOSED, and this section is its record rather than the run to read
now.** The heading said "the current 7.0 baseline" until 2026-08-12 and had been
wrong since P14 landed: a new probe id is a new instrument, so `probe_set` has moved
three times since — `a131e1f4b46d6c83` → `94d64d8bbacf1174` (P14) →
`82a8e2198e54626a` (P15) → `e79f5fcd86a2e5f0` — each move closing the era before it
(§15.44's era-close law, tabulated at §13). "Thirteen probes" was true on the date in
the heading; the roster above now carries fifteen. **The 7.0 run to read now is
[`linux-7.0-2026-08-07-2b44c17-tier3.json`](doctor/linux-7.0-2026-08-07-2b44c17-tier3.json)**
(binary `2b44c1700d17`, probe set `e79f5fcd86a2e5f0`, **25 supported · 0 degraded ·
0 unsupported · 1 skipped**), with its passive twin from the same build;
`docs/doctor/README.md` indexes both and carries the era boundaries. Everything below
in this section stays comparable **with itself** — a closed era's artifacts stay
comparable with each other, no later capture joins them, and nothing here may be
diffed field by field across the boundary without the mismatch stated.

**Tier 3, and it was the 7.0 run to read at the time.** `71fc5a815852` binary, probe set
...
> **A caution for the next reader — discharged 2026-08-12, and replaced by the two
> things that outlived it.** This paragraph used to warn that each probe's per-run
> "Consequence" hardcoded 6.18 as *the* production kernel, so a report generated on
> 6.18 told you to "diff this against the production kernel (6.18)" — i.e. against
> itself. The shipped strings no longer say that. All five that ask for a
> cross-kernel diff — P6, P7, P8, P9 and P10 in `doctor/src/probes.rs` — spell it
> **"the other kernel of record (6.18 or 7.0, whichever this run is not)"**, which
> is self-referential on neither box, and no consequence string names a production
> kernel at all. What survives the fix: those sentences are still unconditional by
> design, so read them against the `kernel` row of the report's own Environment
> table, which is the only thing that knows which of the two this run is. And the
> diff all five ask for **has now been taken** — `linux-6.18-2026-08-07-3e23c52-tier3.md`
> against the `7cf0338` 7.0 triple, one doctor source on both sides — with nothing
> differing across the probe sections, so all five are discharged and none licenses
> a change (`docs/doctor/README.md`; notes §3.73).
