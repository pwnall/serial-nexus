...
**Hand-announcement, 2026-08-21 (design §15.69 clause 1, plan §18 item 92): P5's handshake
*sentence* changed for the all-`false` case, under an unchanged digest pair.** A bench whose eight
cells all read `false` used to print `3-wire: no handshake lines carried` and now prints
`no handshake crossing read: 3-wire, or a transport that manufactures these lines — this reading
cannot separate them`; the DTR-inconclusive sibling moved the same way. **The cells are unchanged,
the classification is unchanged, and every other arm is unchanged** — a 3-wire bench is still a
3-wire bench and still skips the same tests. What moved is what the sentence *claims*, because a
bare input and a transport-manufactured constant low were measured bit-identical. This is exactly
the residual named above: the value moved and no key did, so **neither digest marks it and only this
paragraph does.** Artifacts captured before this date carry the old wording and are frozen (§16.13);
do not read a difference between an old row's sentence and a new one's as a change in the bench.
The affected rows below are every `3-wire: no handshake lines carried` and every
`3-wire: no RTS/CTS crossing,` in this index.
...
## A row may say what a gate answered *then*, never what it answers *now*

Several rows below record that `jq -e -f expectations/<platform>.jq` exited 0 over a
day's artifacts. Read every one of those as **dated**: it is a statement about the
expectation file as it stood when the capture was taken, and it does not survive the
next clause added to that file. The expectation files track the **current tree**;
`docs/doctor/` holds frozen artifacts (§16.13). A clause that requires a cell a later
probe emits will therefore fail every artifact captured before it, and that is the
designed behaviour rather than a broken gate — CI runs `jq -e -f expectations/*.jq`
only against the report it has just produced — `.github/workflows/ci.yml`, the
`Capability report` step of the `doctor` job and the `Capability report + macos.jq gate`
step of the `macos` job. **Both are cited by their `- name:` and not by line, because a
line number into a file that grows has a shelf life measured in commits.** This very
citation gave line numbers when it was written earlier in the same session, and one of
them was stale before the session ended (measured 2026-08-21: the linux `jq -e` line was
still where it said, the macos one three lines below it) — which is the defect
`ci.yml`'s own `Packaging gate (root arm — item 31's owed measurement)` comment already
records against three other citations into this same file. And
`itest/tests/meta_doctor_artifacts.rs` indexes the committed artifacts by digest and
never re-gates them with these files.

**Measured 2026-08-21, so the scale is on the record rather than assumed.** Against
`expectations/*.jq` as they stand at `800915b`, 29 of the 76 `linux-*.json` here and
15 of the 32 `macos-*.json` exit 0. Against the working tree's files, carrying plan
§18 items 73 and 85's new clauses, that becomes **13 of 76** and **6 of 32** — the
passing Linux set being exactly the twelve passive captures plus this session's
Tier-3 one. *(**Re-measured 2026-08-22**, once the clean `4125348` triple joined the
directory: **16 of 79** `linux-*.json` and **6 of 32** `macos-*.json` exit 0 against
`expectations/*.jq` at `4125348`. The delta is exactly those three new files and
nothing else — the same twelve passive captures and the same
`800915b-dirty-tier3` one still pass, and the `-dirty-wireflow-` triple still does
not, for the one reason its row gives. Both counts are over the working tree's
files — 79 `linux-*.json` and 32 `macos-*.json`; `git ls-files` lists 76 and 32 of
them, the three `2026-08-22` captures landing in the same commit as this
sentence.)* The remedy for a row that has gone stale is to date its sentence, never
to soften a clause so a frozen artifact passes: a clause weakened until history
clears it asserts nothing about the report CI is actually gating.

## Index

| File | Kernel / box | Binary | Probe set | Field set | Rig | Verdicts |
|---|---|---|---|---|---|---|
| [`linux-7.0-2026-08-22-4125348-tier3.json`](linux-7.0-2026-08-22-4125348-tier3.json) | 7.0.0-30-generic, Ubuntu 26.04 — the dev box, 20 cores. **The first clean-stamped capture of this bench carrying plan §18 item 85's `wire_flow_control` block**: `build.commit` is `412534867464` with **no `-dirty`**, so §13's comparability ladder rung 2 (*same binary*) is available to this row and to no earlier capture of the two `wire_flow_control` cells. **Narrowed twice on 2026-08-22, and the second narrowing is the lesson.** An earlier draft headlined this row *the first capture of this bench from a clean tree*, which is false — the fixture has many earlier clean-stamped captures. The repair for that asserted *nine earlier clean-stamped captures*, naming them, and **that was false too**: the count omitted the `macos-24.6.0-2026-08-21-3a39896-ftdi5w-tier3{,-2,-3}` triple, which carries both adapter serials, reads `3a39896ca7cc` with no `-dirty`, and was taken earlier the same day. Measured exhaustively over `docs/doctor/*.json` on 2026-08-22: **19** artifacts name both serials, **15** of them clean-stamped, so **12** clean-stamped ones precede this triple. **An exhaustive enumeration is a superlative wearing a number**, and it fails the same way — by a member nobody thought to look for. So this row no longer carries one. What it claims instead is settled by a single search over the whole directory rather than by a list somebody assembled: **`wire_flow_control` appears in exactly six artifacts, three of them the `-dirty-wireflow-` triple below and three of them these; `wire_flow_control.released_intact` appears in these three and nowhere else.** Both are `grep -l` over `docs/doctor/*.json`, and a new capture changes the answer without anyone having to remember this sentence. **It is also the first artifact carrying plan §18 item 73's software cross-check and item 85's wire reading together from a clean build, and the only artifact anywhere carrying `released_intact`** — measured over `docs/doctor/*.json` on 2026-08-22: seven files carry `software_flow_control.shipped_predicate_agrees`, six carry a `wire_flow_control` block, and these three alone carry `wire_flow_control.released_intact`. **Item 85's bound is closed by it.** The `-dirty-wireflow-` triple below was taken before the final repair round added that field, so those three license *1024 bytes back*; these three license *the payload's own continuation back*. **Six cells of six** — both named ports, all three captures — read `reading: "gated"`, `honoured_on_the_wire: true`, `released_intact: true`, `flag_on_peer_not_ready.bytes_delivered_to_the_peer: 0` of `payload_bytes: 1024`, `released_after_peer_raised_rts: 1024` and `cts_after_release: true`, against `control_flag_off_peer_not_ready` and `control_flag_on_peer_ready` both delivering 1024 with `delivered_intact: true`. The cell's own `why` says it: *“0 of 1024 bytes crossed while the peer held RTS low, and 1024 arrived once the peer raised it — byte-for-byte the payload's continuation — against 1024 and 1024 in the two control cells on the same wire.”* **Scope, because the reading licenses exactly this and the cell's `does_not_license` says so too**: this adapter pair on this cable — P4 resolves both to `usb:0403:6001`, an FT232R — on Linux 7.0.0-30-generic, at `baud: 115200`, with a 1024-byte payload and `receive_window_ms: 300`. One driver's answer, and nothing about any other. (The report names no kernel *driver*, so do not read one out of it.) **`field_set` moves and `probe_set` does not**, which is §13's era law clause 4: **no era closes**. The move is `9a75a9f83a83a617` → `b73dba8f32301a84` against the `-dirty-wireflow-` triple below, and it decomposes with nothing left over: **+2 leaf paths, 621 → 623**, one `wire_flow_control.released_intact` per named port, **none removed** — re-measured 2026-08-22 by replaying `report.rs`'s `field_paths` over both files, which reproduces the 573 the `800915b-dirty-tier3` row below publishes. **Unlike that row's diff this one carries no port-naming confound**: both triples were handed the same two `/dev/serial/by-id/…` strings, so every port-keyed prefix matches and the residue is the two new paths alone. `serial-nexus-doctor --field-set` recomputes each of the three to the digest it emitted. **P5 reads `5-wire crossover: RTS/CTS both ways, DTR moves nothing`, identical in all three**, and **all six DTR crossings are `false`** — this is the **same** bench and the same cabling as the `-dirty-wireflow-`, `800915b-dirty-tier3` and `25d8ecd` rows below — the same handshake string in every one of them — re-read, so it is **not** a new cabling for item 28's count and must not be added to it. P14 reads `max_reliable_baud=3000000` `adapter-refused` with `icounts_measurable=true` and `search_elapsed_ms=22020`/`21998`/`22006`. `jq -e -f expectations/linux.jq` exited **0** on all three, measured 2026-08-22 against that file as it stands at `4125348` — read that as dated, per the section above. **Naming**: `generated_utc` is `2026-08-22T05:18:30Z`, and the filename carries that same **UTC** date, so these three are **not** among the day-ahead captures the row below counts. The operator recorded box load 5.32 at capture; no probe emits load average, so that figure is the operator's and not this artifact's | `412534867464` | **`4317ea5ac187f506`** | `b73dba8f32301a84` | **Tier 3** — the FT232R pair (`BH00L4KU` ↔ `BH00LW9U`), **5-wire, measured**, named to the tool by their `/dev/serial/by-id/…` symlinks (the string P5's and P15's observation keys carry, and the one P3's title carries) | 28 supported · 1 skipped |
| [`linux-7.0-2026-08-22-4125348-tier3-2.json`](linux-7.0-2026-08-22-4125348-tier3-2.json) | ” — second sequential run, same box, same build; `generated_utc` `2026-08-22T05:19:08Z` | ” | ” | ” | ” | ” |
| [`linux-7.0-2026-08-22-4125348-tier3-3.json`](linux-7.0-2026-08-22-4125348-tier3-3.json) | ” — third sequential run; `generated_utc` `2026-08-22T05:19:46Z`. All three share a `field set`, the six `wire_flow_control` cells read identically across them, and the eight P5 handshake cells are identical in all three (plan §3 rule 14: three runs, because the quantity varies) | ” | ” | ” | ” | ” |
| [`linux-7.0-2026-08-21-800915b-dirty-wireflow-tier3.json`](linux-7.0-2026-08-21-800915b-dirty-wireflow-tier3.json) | 7.0.0-30-generic, Ubuntu 26.04 — the dev box, 20 cores. **The first artifact anywhere carrying P15's `wire_flow_control` block** — plan §18 item 85's reading, taken by the instrument that item asks for. Both named ports, all three captures: `reading: "gated"`, `honoured_on_the_wire: true`, 0 of 1024 delivered while the peer held RTS low, 1024 on release, and 1024 in each of the two control cells on the same wire. **Superseded 2026-08-22 by `linux-7.0-2026-08-22-4125348-tier3{,-2,-3}.json` at the top of this index, and kept rather than deleted** — the `8c00078-dirty` rows below record the same limitation and the same choice. Two things these three cannot say and the clean triple can. **(i)** The `-dirty-` is `doctor/build.rs`'s stamp for a binary built from a tree carrying uncommitted changes (all three read `800915bf4078-dirty`), unavoidable for a capture whose instrument was that session's own in-flight change, and what it denies is **rung 2 of §13's comparability ladder, *same binary*** — not the reading, which is three sequential runs of one bench. **(ii)** They **predate `released_intact`**, which the final repair round added after they were taken, so a release here is a *count* and not a *content*: what they license is 1024 bytes back, not the payload's own continuation back. That single field is the whole `field_set` difference — `9a75a9f83a83a617` → `b73dba8f32301a84`, +2 leaf paths (621 → 623), one per named port, none removed, re-measured 2026-08-22. `probe_set` does not move and **no era closes**. **`jq -e -f expectations/linux.jq` exits 1 on all three against the working tree at `4125348`, and the sole cause is the clause requiring `wire_flow_control.released_intact` to be a boolean**: measured 2026-08-22 by deleting that one clause and re-running, which turns all three to exit 0. That is the designed behaviour of a frozen artifact under a later clause (§16.13) and not a broken gate — see the section above; the remedy was the re-capture, never a softened clause. P5 reads `5-wire crossover: RTS/CTS both ways, DTR moves nothing` with **all six DTR crossings `false`**, the same bench and the same cabling as the `4125348` triple above and the `800915b-dirty-tier3` and `25d8ecd` rows below — the same handshake string in all of them, and **not** the same cabling as the `8c00078-dirty` rows further down, which read `3-wire` on a bench that shares only one of these two adapters (see the correction on the `25d8ecd` row) — so **not** a new cabling for item 28. P14 reads `max_reliable_baud=3000000` `adapter-refused`, `icounts_measurable=true`, `search_elapsed_ms=22005`/`22040`/`22012`. `generated_utc` is `2026-08-22T01:13:45Z` against a filename dated `2026-08-21`, so these three **are** day-ahead captures in the row below's sense | `800915bf4078-dirty` | **`4317ea5ac187f506`** | `9a75a9f83a83a617` | **Tier 3** — the FT232R pair (`BH00L4KU` ↔ `BH00LW9U`), **5-wire, measured**, named to the tool by their `/dev/serial/by-id/…` symlinks (the string P5's and P15's observation keys carry, and the one P3's title carries) | 28 supported · 1 skipped |
| [`linux-7.0-2026-08-21-800915b-dirty-wireflow-tier3-2.json`](linux-7.0-2026-08-21-800915b-dirty-wireflow-tier3-2.json) | ” — second sequential run, same box, same build; `generated_utc` `2026-08-22T01:14:23Z` | ” | ” | ” | ” | ” |
| [`linux-7.0-2026-08-21-800915b-dirty-wireflow-tier3-3.json`](linux-7.0-2026-08-21-800915b-dirty-wireflow-tier3-3.json) | ” — third sequential run; `generated_utc` `2026-08-22T01:15:01Z`. All three share a `field set` and all three are superseded by the `4125348` triple above for the reasons its lead row gives | ” | ” | ” | ” | ” |
| [`linux-7.0-2026-08-21-800915b-dirty-tier3.json`](linux-7.0-2026-08-21-800915b-dirty-tier3.json) | 7.0.0-30-generic, Ubuntu 26.04 — the dev box, 20 cores. **The capture plan §18 item 73 owes**: the software flow-control cross-check, executed against real hardware rather than against injected rows. P15 reports `software_flow_control.shipped_predicate_agrees: true` on **both** named ports — `serial_nexus_sys::honours_flow_control(.., FlowMode::XonXoff)`, the predicate `load` consults since §15.61, reached the same three-way arm as the read-back this probe took by hand, on a driver that honours the mode (`c_iflag` `0x5` → `0x1405`, a delta of exactly `IXON`/`IXOFF`, on both ports). **This row is the anti-vacuity evidence and nothing more**: it shows the cell is computed on a live port by a predicate that really opened it, not that the cell can go `false` — no driver in this record perturbs a third `c_iflag` bit, so the disagreeing arm was proven by a planted divergence and by injected rows instead (that plant read `shipped_predicate_agrees: false` with `ixon`/`ixoff` both `true` on both ports, and is **not** committed). **`field_set` moves and `probe_set` does not**, which is §13's era law clause 4: **no era closes**. The move is `64eb252e565113b2` → `f18630922c4eecc7`, and **both ends were measured on this tree, on this bench, in this session** — a pre-change capture taken an hour earlier from the binary this change was then made to, and this one. **That pre-change capture is not committed**, so it is named here as provenance rather than offered as evidence; what makes the move checkable anyway is that its decomposition can be reproduced from *this* row's file alone. It decomposes with nothing left over: **+2 leaf paths, 571 → 573**, one `software_flow_control.shipped_predicate_agrees` per named port, none removed — and deleting exactly those two paths from this artifact's own sorted, deduplicated leaf-path set and re-running the `field set` digest over the remaining 571 yields `64eb252e565113b2`, while leaving them in yields this row's own `f18630922c4eecc7` (recomputed 2026-08-21). **No delta may be derived against the `25d8ecd` row above.** It is a different commit, which §13's comparability ladder settles on its own — and the two `field set` digests are incomparable for a second reason that has nothing to do with the instrument: **the two runs named their ports differently.** The `25d8ecd` run was given `/dev/ttyUSB0` and `/dev/ttyUSB1`, this one the `/dev/serial/by-id/…` symlinks for the same two adapters, and this digest keys every P5, P11 and P15 leaf path by the string it was handed (`field set` design choice 4 — nothing is excluded by name). Measured over the two files as they stand in the working tree (one of them still untracked — see the provenance note that closes this cell): **82 leaf paths present in this row's `800915b-dirty` artifact and absent in the `25d8ecd` one, and 80 in the other direction** (573 paths against 571, over an intersection of 491 — re-measured 2026-08-21 by replaying `report.rs`'s `field_paths` over both files). **An earlier draft of this sentence gave one number, 80, “in each direction”**: a symmetric sentence about a measurement that is not symmetric, and the two paths the symmetry hid are the two this row exists to report, and once every port string is rewritten to a common name the residue is exactly the two `software_flow_control.shipped_predicate_agrees` cells. So a digest diff across these two rows carries no instrument statement at all, and the `+2` above belongs to the pre-change pair and to that pair alone. **An earlier draft of this row justified the same conclusion by saying the `25d8ecd` observation set carries a `recheck.refill_reproduced_total` this run did not produce. Both artifacts carry it** — the same two leaf paths, `P10.master_to_slave_hostward.recheck.refill_reproduced_total` and `P10.slave_to_master_targetward.recheck.refill_reproduced_total`, in each file. A correct conclusion resting on an invented observation is the failure this directory exists to make checkable, and it is recorded here rather than quietly deleted. P5 reads `5-wire crossover: RTS/CTS both ways, DTR moves nothing`, and **all six DTR crossings are `false`** — this is the **same** bench and the same cabling as the `25d8ecd` row, re-read, so it is not a new cabling for item 28's count and must not be added to it. Named by the operator's **local** session date, as are the **31 committed captures** in this directory whose `generated_utc` runs a day ahead of their filename (recounted 2026-08-21 over `git ls-files docs/doctor/*.json` — 104 committed artifacts, 31 of them a day ahead; the three uncommitted `-wireflow-` captures taken on this bench in this same session carry the same offset and are not in that count); its `generated_utc` is `2026-08-22T00:40:33Z`. **Provenance, plainly, because two different things are easy to conflate here.** The `-dirty` is the suffix `doctor/build.rs` stamps when the tree the binary was built from carried uncommitted changes: this capture came from a working tree holding item 73's change *and* other in-flight work, so the binary string is not a claim that the tree was `800915b` plus this item alone, and **§15.44 rung 2's same-binary claim is therefore not available for this row** — the `8c00078-dirty` triple further down records the same limitation and the remedy it used, which is to state a scoped source diff instead. Separately, and independently of the stamp: **at the time this row was written the artifact itself was untracked** (`git ls-files` does not list it). It must land in the same commit as this row; until it does, no claim elsewhere may cite it as the committed artifact AGENTS §7 requires, and a re-capture from a clean tree would retire the `-dirty` caveat altogether. **Annotated 2026-08-22 — superseded, and three sentences of this cell are now discharged or stale.** The re-capture it names as the remedy exists: `linux-7.0-2026-08-22-4125348-tier3{,-2,-3}.json` at the top of this index — same bench, same cabling, `build.commit` `412534867464` with **no `-dirty`**, `probe_set` unchanged at `4317ea5ac187f506` so **no era closes**. **(i)** This artifact is committed; `git ls-files docs/doctor/*.json` lists it, so the *until it does* clause above is paid and the AGENTS §7 citation is available. **(ii)** The three `-wireflow-` captures this cell calls uncommitted are committed too, and are indexed immediately above. **(iii)** The day-ahead recount therefore moves: **35 of 108** committed artifacts run a day ahead of their filename, not 31 of 104 — re-measured 2026-08-22 over `git ls-files docs/doctor/*.json`, the four newly-committed files all being day-ahead, and the three `2026-08-22` captures **not** among them because their filenames carry their own UTC date. Everything else here stands, including what the `-dirty` stamp denies and does not: rung 2 of §13's ladder, *same binary*, and nothing more. **This row also keeps an evidentiary job the clean triple cannot take from it**: it is the only capture in this directory carrying item 73's `software_flow_control.shipped_predicate_agrees` and **no** `wire_flow_control` block at all (measured 2026-08-22 over `docs/doctor/*.json`: seven files carry the first, six the second, this one alone carries the first without the second), which is why its `571 → 573` decomposition is stated here and is not restated above | `800915bf4078-dirty` | `4317ea5ac187f506` | `f18630922c4eecc7` | **Tier 3** — the FT232R pair (`BH00L4KU` ↔ `BH00LW9U`), **5-wire, measured**, on `/dev/ttyUSB1`/`ttyUSB0` | 28 supported · 1 skipped |
| [`linux-7.0-2026-08-21-25d8ecd-tier3.json`](linux-7.0-2026-08-21-25d8ecd-tier3.json) | 7.0.0-**30**-generic, Ubuntu 26.04 — the dev box, 20 cores. **The FT232R pair read on Linux hours after the same two adapters and the same cable were read on Darwin** (`macos-24.6.0-2026-08-21-3a39896-ftdi5w-*` above), so hardware and cable are held fixed across the kernel. **P5 reads `5-wire crossover: RTS/CTS both ways, DTR moves nothing`, identical in all three runs**, and **all six DTR crossings are `false` — item 28 stays blocked for a fifth cabling.** These are the artifacts design §15.69 clause 1 rests on for the *5-wire* half of its bit-identical argument: the six unconnected DSR/DCD/RI inputs on this bench read `false` at both peer drive levels, which is what a bare FT232R input does, and it is what Apple's CDC-ACM stack manufactures. The 3-wire half is `linux-7.0-2026-08-13-8c00078-dirty-tier3*`, whose eight cells all read `false` on a genuine 3-wire bench of this same pair. *(**Corrected 2026-08-22: “this same pair” is wrong and the rest of the sentence is right.** That bench is `ABSCDGL6` ↔ `BH00L4KU` — read out of its own P4 rows, `usb:0403:6001:ABSCDGL6:00` and `usb:0403:6001:BH00L4KU:00`, which are also the strings its P5 keys spell — so it shares exactly **one** adapter with this row's `BH00L4KU` ↔ `BH00LW9U`, not both. **Nothing rests on the difference**: design §15.69's clause 1, plan §18 item 92 and notes §3.121 all cite that capture as *a genuine 3-wire FT232R bench*, which it is, and none of them claims the pair is held fixed — the all-eight-`false` reading and the bit-identity argument stand unchanged. Recorded rather than deleted, because a claim that upgraded “same device class” into “same pair” is exactly the kind this index exists to make checkable.)* **P14 reads `max_reliable_baud=3000000` `adapter-refused` with `search_elapsed_ms=22086`** — 12 % of `P14_BUDGET`, which is what made item 98's comparison affordable. `icounts_measurable=true`. **Both digests are unchanged from the pre-change capture of the same bench, so no era closes and no field moves**; the handshake *wording* change of §15.69 clause 1 is a value this bench never produces, and the announcement it owes is above | `25d8ecd39aed` | **`4317ea5ac187f506`** | `29e4071d23e8b474` | **Tier 3** — the FT232R pair (`BH00L4KU` ↔ `BH00LW9U`), **5-wire, measured**, on `/dev/ttyUSB0`/`ttyUSB1` | 16 supported · 1 skipped |
| [`linux-7.0-2026-08-21-25d8ecd-tier3-2.json`](linux-7.0-2026-08-21-25d8ecd-tier3-2.json) | ” — second sequential run, same box, same build | ” | ” | ” | ” | ” |
| [`linux-7.0-2026-08-21-25d8ecd-tier3-3.json`](linux-7.0-2026-08-21-25d8ecd-tier3-3.json) | ” — third sequential run; the eight handshake cells are identical in all three, and `jq -e -f expectations/linux.jq` exited 0 on all three **against that file as it stood at `25d8ecd`** — re-measured 2026-08-21 and confirmed, exit 0 on all three. **Against the working tree carrying plan §18 item 73's clause all three now exit 1**, because that clause requires `software_flow_control.shipped_predicate_agrees` to be *present* and this build emitted no such key. Nothing is broken by that; see the dated-gate note above the index | ” | ” | ” | ” | ” |
| [`macos-24.6.0-2026-08-21-3a39896-ftdi5w-tier3.json`](macos-24.6.0-2026-08-21-3a39896-ftdi5w-tier3.json) | Darwin 24.6.0 / **macOS 15.8 (24H16)**, x86_64 — the same rig box as the CDC-ACM rows below, **same session, same binary, and the same physical cable**; only the adapters changed. **The extra `ftdi5w` segment is deliberate**: these share a UTC day *and* a commit with those rows, and the `-2`/`-3` index is reserved for sequential runs of one binary on one bench — using it here would assert 'same instrument, run again, so a difference is noise' about a different bench, which is exactly the claim this directory's naming rule exists to prevent. **This is the decisive control for design §15.68.** **P5 reads `5-wire crossover: RTS/CTS both ways, DTR moves nothing`** — `rts_a_to_cts_b=true rts_b_to_cts_a=true`, all six DTR crossings `false` — on the cable that, one fixture earlier, made P5 print `3-wire: no handshake lines carried`. **So that cable is a measured 5-wire crossover and the CDC-ACM sentence was wrong about its own subject** (plan §18 item 92); it does *not* decide whether the WCH module's header or Apple's CDC stack caused the reading. **Second control, and the one that scopes §15.68 clause 3:** the identical loss-and-duplication probe that reads 8 lost and 8 duplicated per 128 records on the CDC-ACM bench reads **128 of 128 distinct, 0 lost, 0 duplicated, 5 of 5** here — same box, same hubs, same cable, same payload and rate — so that defect is neither USB topology nor the harness nor Darwin generally. **P15 reads `honoured_on_readback: false`, `silently_dropped: true`, `c_cflag 0x4b00 → 0x4b00` on both ports** (the standalone control reads `0xcb00 → 0xcb00`, differing only because it sets `CLOCAL`; both are a delta of zero), reproducing §15.53's FT232R finding on this OS and confirming Apple drops `CRTSCTS` on both device classes measured here. P14 reads `max_reliable_baud=1906250` `unreliable-timed-out`; **the triple disagrees (1906250 / 2687500 / 1906250), so the number is noisy on this bench and only the order of magnitude is quotable** | `3a39896ca7cc` | **`4317ea5ac187f506`** | `d4b27b9ebf82f66e` | **Tier 3** — the FT232R pair (`BH00L4KU` ↔ `BH00LW9U`), **5-wire, measured**. The same pair AGENTS §2 records reading `true`/`true` on Linux on 2026-08-14 | 8 supported · 8 degraded · 1 skipped |
| [`macos-24.6.0-2026-08-21-3a39896-ftdi5w-tier3-2.json`](macos-24.6.0-2026-08-21-3a39896-ftdi5w-tier3-2.json) | ” — second sequential run, same bench, same build | ” | ” | ” | ” | ” |
| [`macos-24.6.0-2026-08-21-3a39896-ftdi5w-tier3-3.json`](macos-24.6.0-2026-08-21-3a39896-ftdi5w-tier3-3.json) | ” — third sequential run. **All three agree byte-for-byte on the P5 handshake line**; `max_reliable_baud` does not, which is what the triple is for (plan §3 rule 14) | ” | ” | ” | ” | ” |
| [`macos-24.6.0-2026-08-21-3a39896-tier3.json`](macos-24.6.0-2026-08-21-3a39896-tier3.json) | Darwin 24.6.0 / **macOS 15.8 (24H16)**, x86_64 — the same MacBookPro15,1 rig box as the `b346188` rows below, on a newer OS build (those read 15.7.8). 12 cores. **The fourth cell of the {Linux, Darwin} × {FTDI, CDC-ACM} square, and the reason design §15.68 exists.** **P5 reads `3-wire: no handshake lines carried` on all eight crossings — on the bench the operator reports as 5-wire, and the bench the Linux rows above call `UNREADABLE handshake … this is not a 3-wire answer`.** Both sentences describe one physical bench and both cannot be licensed: Linux's `cdc_acm` manufactures CTS **high**, which is self-refuting and reaches the `UNREADABLE` arm, while Apple's stack leaves it **low**, which `crosses()` has no word for because it is bit-identical to an absent conductor. **This is the first artifact in this directory where §15.62's reading-keyed repair falls through** (plan §18 item 92). Two further readings this triple is the first artifact for. (a) **P14 reports `max_reliable_baud=14400`** with `ceiling_kind=unreliable-timed-out` and `first_unreliable_baud=15000` — **and that is not this pair's maximum rate**: the same adapters read 6 000 000 on Linux, and 240-byte payloads are byte-exact here at every standard rate to 921600. The number is two probe policies meeting two transport defects — the constant-airtime payload (`baud/40`) handed the 19200 rung 480 bytes, which the block-substitution defect breaks, and refinement then landed every midpoint on a line coding this stack accepts, echoes exactly, and does not carry (15000/15600/16800 received 1, 0 and 0 bytes of their full payloads). **Do not quote 14400 as a rate ceiling.** (b) **P15 reads `honoured_on_readback: false`, `silently_dropped: true`, `shipped_predicate_agrees: true`, `c_cflag 0x4b00 → 0x4b00` on both ports** — so Apple's `IOSerialFamily` now drops `CRTSCTS` on **two device classes**, the FT232R of §15.53 and this one, and §15.53's refusal fires here where it does not on `cdc_acm`. `icounter=false` throughout: `TIOCGICOUNT` is Linux-only, so the break and parity counters cannot be read on this kernel however real they are | `3a39896ca7cc` | **`4317ea5ac187f506`** | `b3606b2993728740` | **Tier 3** — the WCH `1a86:55d3` **CDC-ACM** pair (`5A7C297954` ↔ `5A7C298854`), `AppleUSBCDCCompositeDevice` → `AppleUSBACMData` → `IOSerialBSDClient`. **The same two adapters, on the same cable, as the `linux-7.0-2026-08-17-a7e6070` rows above** — so this pair holds hardware and cable fixed and moves only the kernel and driver | 7 supported · 9 degraded · 1 skipped |
| [`macos-24.6.0-2026-08-21-3a39896-tier3-2.json`](macos-24.6.0-2026-08-21-3a39896-tier3-2.json) | ” — second sequential run, same box, same build | ” | ” | ” | ” | ” |
| [`macos-24.6.0-2026-08-21-3a39896-tier3-3.json`](macos-24.6.0-2026-08-21-3a39896-tier3-3.json) | ” — third sequential run. **All three agree byte-for-byte on the P5 handshake line and on `max_reliable_baud`**, so neither is noise (plan §3 rule 14) | ” | ” | ” | ” | ” |
| [`macos-24.6.0-2026-08-21-3a39896-passive-1.json`](macos-24.6.0-2026-08-21-3a39896-passive-1.json) | ” — **the passive half of the same binary**, taken before the Tier-3 triple, so this bench has both halves from one build. Sixth committed instance of the §5 folding declination: same `probe set`, different `field set`, one binary. `jq -e -f expectations/macos.jq` was **executed** against all six and exited 0 **against that file as it stood at `3a39896`** (dated, per the note above the index) | `3a39896ca7cc` | **`4317ea5ac187f506`** | `83bdc79c944b8534` | none (passive; the adapters are attached but unnamed, so P3/P5/P11/P14/P15 skip) | 5 supported · 5 degraded · 6 skipped |
| [`macos-24.6.0-2026-08-21-3a39896-passive-2.json`](macos-24.6.0-2026-08-21-3a39896-passive-2.json) | ” | ” | ” | ” | ” | ” |
| [`macos-24.6.0-2026-08-21-3a39896-passive-3.json`](macos-24.6.0-2026-08-21-3a39896-passive-3.json) | ” | ” | ” | ” | ” | ” |
