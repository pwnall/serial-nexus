...
| `advertised_baud` (pty) | `1 ..= 4294967295` | `baud`'s window, deliberately: a pty's advertised rate is cosmetic (§7.2) but it is still a rate, and a nonstandard one is *skipped rather than approximated*, so the ceiling stays the field's own type. `0` is refused because it is not a rate in either place. Until this bound landed the field was safe only by accident — the standard-rate lookup answers `None` for it, so nothing was ever set — and §16.12 asks a numeric attribute for a stated bound rather than for an accident three modules away |
