...
**Status:** Executed — phases 0–8, the simplification track (§9), the extension track (§10), and web-console items §11.1–§11.6 are complete, audited, and green; macOS is runtime-verified on real hardware and the validation suite is the Rust `nexus-itest` crate (§15.30, §15.31). §11.7–§11.9, §16.11, the console-map track (§12), and the review-26 remediation track (§13 — all 93 findings dispositioned, ledger in `docs/27-review-26-remediation-ledger.md`) are executed, as is the graph-editing track (§14) and the flake-remediation session (design §15.36 — suite at 480, four consecutive clean runs, one residual load-sensitive lead recorded); open work: the browser-UI automation track (§15, from design §15.37).
...
1. **The map node.** `type = "map"` with ordered `hostward`/`targetward` mapping lists over the picocom vocabulary; stateless first-match substitution, bounded expansion, per-rule counters in state, `held` default on the targetward edge, host-facing side as the default endpoint with the standard lock/ring/tap machinery. *Validation:* every mapping round-trips against an independent in-test oracle in `nexus-itest`, including CR and LF split across chunk boundaries (trivially safe by statelessness, pinned anyway); an ordering test proves first-match determinism (`igncr` before versus after `crlf` differ as documented); a proptest bounds output at k× input and holds the holdover bound under injected Busy; an unknown mapping name fails load structurally with the name in the error; the steal-to-bypass e2e shows mapped bytes at the map's endpoint and raw bytes upstream reaching the sim device verbatim; the map's default ring splices exactly and its per-rule counters match the oracle's tallies.
2. **Reference configuration and docs.** `serialnexusd.example.toml` gains a mapped console (bare-LF quirk normalized hostward, CR expectation satisfied targetward); the configuration docs describe the vocabulary, the ordering rule, and the picocom correspondence; `docs/rpc` state pages document the per-rule counters. *Validation:* the example config load-verifies with the map present; the docs suite links the new sections; the web console shows the mapped stream for the example's quirky console without any client-side settings — the stated motivation, observed.

## 13. Review-26 remediation track

*Executed in full.* Design §15.34 carries the pattern-level record; `docs/27-review-26-remediation-ledger.md` maps all 93 finding ids to dispositions, including the deliberate declines — consult it before re-filing anything from that review, because silently re-fixing a declined item is its own defect. The class-prevention gates now standing: structural range checks on every numeric attribute (with proptests over the out-of-range space), `deny_unknown_fields` across the schema, the bridge's parse-one-request/allowlist screen with smuggling regressions, capped-and-counted remote-fed collections, `tap.closed` lifecycle tests, and the `-32006` pipelining rule's four-test battery. Suite at 435.

## 14. Graph-editing track

Design §15.35 is the rationale. Four items, agent-validated.

1. **The `ports` verb.** Passive resolver enumeration — identity, current path, bound-status — via by-id/sysfs readlinks (macOS: `cu.*` scan), never `open(2)`. *Validation:* against a fixture dev-root, `ports` lists exactly the unbound candidates and flags the bound one; adding a listed port then re-running flips its status; a provider-level assertion (and an strace spot-check on the checklist) proves no candidate device node is opened; `serialnexusctl ports` renders it.
2. **`connect`/`disconnect`.** Edge surgery under the same critical-section validation as load: orientation pairing, single-edge rule, mux-edge write-mode corollary, held-uniqueness — each violation a named structural error with nothing changed. *Validation:* a mid-stream `connect` of a new log edge captures the stream from the live edge onward (checksum from join point); `disconnect` of a lock-holding origin releases the lock and purges (the phantom-holder regression, edge-op variant); `disconnect` of a codec's held mux edge stalls channels exactly like a steal would; every illegal connect is refused naming the rule; dump round-trips after a connect/disconnect sequence.
3. **Web graph page + indicators.** Topology rendered from `state`, live status via `subscribe`. *Validation:* API-level — the page's data endpoint equals `state` (structural comparison in `nexus-itest`); a scripted fault (unplug the sim device) flips the node's indicator through `waiting` and back within the poll bound.
4. **Web editor + posture.** Bridge allowlist grows the graph verbs (lifecycle verbs remain refused); the editor page drives add/remove/connect/disconnect with daemon-surfaced errors and cascade-naming confirmations; `docs/security.md` gains the capability statement (graph editing is daemon-user capability; the token is operator trust). *Validation:* the smuggling and unknown-verb regressions stay green with the wider allowlist; `shutdown` through the WS is still refused; an end-to-end run adds a console via the editor page's API path and bytes flow through it; the security page states the exec-codec/log-path implication verbatim.

## 15. Browser-UI automation track

Design §15.37 is the rationale: the browser half of §16.7 becomes CI-verified in real headless Chromium. Four items, agent-validated.

1. **The Playwright scaffold.** `serialnexusweb/ui-tests/` with a pinned `package.json` + lockfile (dev-time only; assets stay vendored, no build step), Chromium project only; launched by a `nexus-itest` gate (`p8_web_ui.rs`) that spawns the suite against a daemon + sim fixture and self-skips visibly without `node`/`npx`; traces and screenshots captured on failure as CI artifacts. *Validation:* the gate runs the suite green on a fixture graph; deleting `node` from PATH yields the standard skip marker, not a failure; a planted failing spec produces a trace artifact.
2. **The behavior suites.** Console page: tapped stream renders, `send` round-trips through the echo oracle, the drop counter surfaces when forced. History: reload mid-stream and assert the stored log equals the source exactly once (the offset splice, asserted *in the browser*), storage badge reflects `persist()` status, export and clear work. Lifecycle: `load --replace` under an open console triggers the `tap.closed` re-anchor without duplication. Graph and editor pages: a sim unplug flips the node indicator through `waiting` and back; adding a console via the editor makes bytes flow end to end; an illegal connect surfaces the daemon's named structural error; `shutdown` through the page's transport is still refused. *Validation:* each spec is fail-first proven against a planted regression (per §15.36 rule 4) where one exists to plant.
3. **CI wiring.** The web job installs the pinned Chromium via `npx playwright install --with-deps chromium` (cached), runs the gate per push, full traces nightly. *Validation:* the job manifest pins versions; a cache-cold run stays under the job timeout; artifacts appear on a forced failure.
4. **Checklist reduction.** The §16.7 manual items for the web client shrink to rendering fidelity (fonts, xterm visuals) and real-rig interaction; the OPFS and re-anchor items move to "CI-verified" with a pointer to the suite. *Validation:* the checklist doc diff shows the moves; the doctor/report language and `docs/security.md` are untouched by the reduction.
