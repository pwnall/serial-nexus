...
//! transform in `nexus-daemon/src/nodes/codec.rs`'s `signal_tests`; what is pinned
...
/// on purpose: `nexus-itest/src/lib.rs` is shared and must not grow per-test helpers.
...
///
/// It speaks the shared envelope by hand (`codec-api`'s v1 framing: a big-endian
