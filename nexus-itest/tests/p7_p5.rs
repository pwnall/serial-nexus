...
//! in the double rather than a scheduling concession: `nexus-sim`'s `pty --echo`
...
/// `nexus-doctor --json --port …`, not yet started.
...
fn run_doctor(ports: &[&str]) -> Value {
    let out = doctor_cmd(ports).output().expect("run nexus-doctor");
    doctor_report(&out)
}
...
///
/// This is the regression guard for the `nexus-sim pty --echo` defect described in
...
fn p5_reports_a_peer_that_hangs_up_mid_probe_as_hung_up_not_dangling() {
    ...
    let out = child.wait_with_output().expect("wait for nexus-doctor");
