...
//!   project's own `nexus-sim pty` — all legal `raw:` devices per §7.1/§12) `set_dtr`
...
// `nexus_itest::Daemon` deliberately does not expose its pid, and `src/lib.rs` is
...
fn a_break_straddled_by_a_replace_leaves_the_line_transmitting() {
    ...
    // the lossless sink AGENTS §5 says to ground a byte claim on.
    let cfg = format!(
        r#"
[[node]]
type = "serial"
