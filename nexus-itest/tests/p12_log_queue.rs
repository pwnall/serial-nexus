...
//! `serialnexusd` under `ulimit -u 1` and asks it about the node.
//!
...
//! guard beside it (`nexus-daemon/src/nodes/log.rs`'s
...
use nexus_itest::{Daemon, Rpc, Sim, TempRun, bin, daemon_answers, serial_echo, wait_until};
...
/// A hand-managed `serialnexusd`, SIGKILLed and reaped on drop. `Daemon` cannot be used
...
fn a_daemon_that_cannot_spawn_the_log_writer_starts_and_faults_the_node() {
    ...
    let script = format!(
        ...
        daemon = bin("serialnexusd").display(),
    ...
    let child = Command::new("bash")
        ...
        .spawn()
        .expect("spawn serialnexusd under a thread limit");
