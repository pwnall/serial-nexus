...
use nexus_itest::{Daemon, serial_echo, sha256_hex, wait_until};
...
"#,
    ...
    const STRUCTURAL: i64 = -32002; // nexus_rpc::AppError::Structural
...
fn disconnecting_a_lock_holding_origin_releases_the_lock_and_purges() {
    ...
    let holder = |rpc: &nexus_itest::Rpc| {
        rpc.node("m")
...
"#,
    ...
    let origins = |rpc: &nexus_itest::Rpc| -> Vec<String> {
        rpc.node("m")
            .and_then(|n| n.pointer("/lock/origins").cloned())
            .and_then(|o| o.as_array().cloned())
            .unwrap_or_default()
