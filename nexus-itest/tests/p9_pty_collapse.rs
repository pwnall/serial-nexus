...
//!    `nexus-sys`'s `SessionLatch` (design §15.39) carries it on Darwin via a kqueue
//!    `EVFILT_READ | EV_CLEAR` knote, and this test runs unskipped on both
//!    platforms. `nexus-doctor` P7 measures the packet mechanism and **P12** the
...
fn a_collapsed_termios_only_session_still_releases_the_write_lock() {
    ...
    // as an *edge* even where no level state records it, so `nexus-sys`'s
    ...
    // failure names the platform's own mechanism, and `nexus-doctor` P7 (`packet
