...
//! — the one subprocess here is `serialnexusctl`, and even there the assertion is its
//! *exit status* plus the surviving graph read back over RPC, never its prose.

use std::process::Command;
use std::time::Duration;

use nexus_itest::{Daemon, Rpc, bin, serial_echo, wait_until};
use serde_json::{Value, json};

/// `nexus_rpc::AppError::Structural` — `APP_ERROR_BASE (-32000)` minus its ordinal
...
"#,
        dev = d.run().join("absent-device").display(),
        ring = nexus_core::config::MAX_REPLAY_RING,
    );
...
fn absurd_hostward_buffer_under_replace_leaves_the_running_graph_intact() {
    ...
    // the RPC as JSON — the same path `serialnexusctl` uses once it has parsed a file,
...
/// `packaging/serialnexusd.example.toml` used to show it: omitted.
...
/// Run `serialnexusctl --socket <sock> load <file> --replace` and return its exit
...
fn ctl_load_replace(d: &Daemon, file: &std::path::Path) -> std::process::ExitStatus {
    Command::new(bin("serialnexusctl"))
        ...
        .arg("--replace")
        .output()
        .expect("run serialnexusctl load --replace")
        .status
}
