//! Review 32, `CTL-1` — `serialnexusctl tap` must **exit** on the terminal
...
//! `tap.data`, so `serialnexusctl tap console > incident.bin` never returned after a
...
//!    macOS, §5). A seeded `nexus-sim` source feeds the endpoint, the CLI captures to a
...
//! lives in one small type instead (`TapContinuity` in `serialnexusctl/src/main.rs`)
...
use nexus_itest::{Daemon, Rpc, Sim, bin, serial_echo, sha256_hex, wait_until};
...
/// Spawn `serialnexusctl --socket <sock> tap <args…>` with stdout and stderr each
...
fn spawn_ctl_tap(d: &Daemon, args: &[&str], out: &Path, err: &Path) -> Child {
    ...
    Command::new(bin("serialnexusctl"))
        .arg("--socket")
        .arg(d.socket())
        .arg("tap")
        .args(args)
        ...
        .spawn()
        .expect("spawn serialnexusctl tap")
}
...
fn wait_exit(child: &mut Child, bound: Duration) -> Option<ExitStatus> {
    ...
    loop {
        match child.try_wait().expect("try_wait on serialnexusctl tap") {
...
/// byte-identical to `nexus-sim`'s output.
...
fn ctl_tap_exits_on_tap_closed_rather_than_blocking_forever() {
    ...
    assert!(
        ...
        "`serialnexusctl tap usb0` never exited after teardown — it is still blocked in \
         read_line on a connection the daemon deliberately keeps alive (CTL-1)"
    );
    assert!(
        capped_status.is_some(),
        "`serialnexusctl tap usb0 --bytes 4096` never exited after teardown (CTL-1)"
    );

    // The documented statuses (see `tap_closed` in serialnexusctl/src/main.rs).
    assert!(
...
"#,
    ...
    assert!(
        status.is_some(),
        "`serialnexusctl tap usb0 > capture` never exited after teardown (CTL-1)"
    );
