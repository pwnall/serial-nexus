...
//!   `serialnexusctl send` reported "sent 20 byte(s)" and `purged_on_reconnect` was 20
...
//! `nexus-sim` still holds is never last-closed. That is why this guard can prove the
...
//! serial device — socat `PTY,link=`, QEMU `-serial pty` and `nexus-sim` alike, all of
...
use nexus_itest::{Daemon, Sim, TempRun, serial_echo, sha256_hex, wait_until};
...
fn the_port_is_still_exclusive_while_the_node_holds_it() {
    ...
    // A second opener is refused while the node is live. `nexus-sim`'s own `client`
