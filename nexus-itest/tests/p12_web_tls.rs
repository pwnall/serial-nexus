...
//! precise — `serialnexusweb/src/tls.rs`'s unit tests, which call `build_config`
...
use nexus_itest::{TempRun, bin, sha256_hex, wait_until};
...
/// pair while the signed cert is not yet installed. Before the fix, `serialnexusweb`
/// read that as "first run", overwrote the key with a throwaway one, printed its
/// bootstrap URL and served normally.
#[test]
fn serialnexusweb_refuses_a_half_present_tls_pair_and_leaves_the_key_byte_identical() {
    let run = TempRun::new();
    ...
    if !exited {
        ...
        let _ = child.wait();
        panic!(
            "serialnexusweb came up with a half-present TLS pair (WEB-1): it must refuse \
             rather than generate over the operator's key"
        );
    }
    let status = child.wait().expect("wait for serialnexusweb");
    assert!(
        !status.success(),
    ...
    child
        .stderr
        .take()
        .expect("piped serialnexusweb stderr")
        .read_to_string(&mut stderr)
        .expect("read serialnexusweb stderr");
    assert!(
