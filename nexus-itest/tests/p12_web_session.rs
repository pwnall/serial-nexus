...
use nexus_itest::{TempRun, bin, daemon_answers, wait_until};
...
impl WebServer {
    /// Spawn `serialnexusweb --bind 127.0.0.1:0 --token <token> --socket <socket>` and
...
/// A `serialnexusd` this test owns outright, so it can be **killed mid-session** — the
/// event WEB-4 is about. [`nexus_itest::Daemon`] deliberately keeps its child private
...
impl OwnedDaemon {
    ...
    /// courtesy close — the routine "the operator restarted serialnexusd" event.
    fn kill(&mut self) {
        let _ = self.child.kill();
        let _ = self.child.wait();
    }
...
/// Mirrors `serialnexusweb/src/server.rs`'s `HEAD_TIMEOUT`. Nothing waits on it here; it
