...
- [`docs/43-design-claude-fable-v17.md`](docs/43-design-claude-fable-v17.md) — the normative design document (concepts, node types, RPC contract, and the reasoning behind each decision). A `§N` reference anywhere in this repository means this document. Superseded generations live in [`docs/historical/`](docs/historical/) and are **not** normative; this bullet and `AGENTS.md` §2 are the two places that name the current pair by filename as normative, so both must be bumped whenever a generation lands (the filename-keyed code sites — `itest/tests/meta_names.rs`'s §15.41 ban-statement allowance and citation-gate scoping pair, and `itest/tests/meta_derive.rs`'s `PLAN`/`DESIGN` consts — also key on the filenames and move with them).
- [`docs/44-implementation-plan-claude-fable-v17.md`](docs/44-implementation-plan-claude-fable-v17.md) — the normative implementation plan (harness doctrine, testing strategy, the Status figures table, and the §18 work ledger).
